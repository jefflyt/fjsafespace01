...
- Next PR (PR-R1-05) dependency is satisfied: APIs exist for frontend to consume
- Test tenant scoping: authenticated user sees only their tenant's sites
...
4. **Standards router** (`backend/app/api/routers/preferences.py` or separate `standards.py`)
...
9. **Register new routers** (`backend/app/main.py`)
   - `app.include_router(preferences_router, prefix="/api/sites")`
   - `app.include_router(interpretations_router, prefix="/api/interpretations")`
...
curl http://localhost:8000/api/sites/{id}/metric-preferences
curl http://localhost:8000/api/interpretations/co2_ppm/WATCH
```
