...
| `priority_logic` | TEXT | Default `'P2'` — priority calculation rule |
| `reference_source_id` | UUID | Nullable FK → `reference_source.id` |
...
| `zone_name` | TEXT | Nullable — zone identifier (e.g. "Lobby"). Populated from CSV. |
...
| `rule_id` | TEXT | Rulebook entry identifier (string reference, not FK) |
...
| `upload` | `ix_upload_content_hash` | Dedup lookup (partial: WHERE parse_status = 'COMPLETE') |
